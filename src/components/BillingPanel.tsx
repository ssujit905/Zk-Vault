        ...
        <div className="space-y-8 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {plans.map((plan) => (
                    <div
                        key={plan.id}
                        className={`relative glass-card p-8 flex flex-col h-full transition-all duration-300 hover:translate-y-[-4px] ${tier === plan.id ? `ring-2 ring-${plan.color === 'primary' ? 'primary-500' : plan.color === 'amber' ? 'amber-500' : 'slate-500'}` :
                            'hover:border-white/20'
                            } ${plan.popular ? 'lg:scale-105 z-10' : ''}`}
                    >
                        {plan.popular && (
                            <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-amber-500 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg">
                                Most Popular
                            </div>
                        )}

                        <div className="mb-8">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${plan.color === 'amber' ? 'bg-amber-500/10 text-amber-500' :
                        ...
                        <button
                            ...
                            className={`w-full py-4 rounded-xl font-black text-xs uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${tier === plan.id
                                ? 'bg-white/5 text-slate-500 cursor-default border border-white/5'
                                : plan.color === 'amber'
                                    ? 'bg-amber-500 text-black hover:bg-amber-400 shadow-[0_4px_20px_rgba(245,158,11,0.3)] active:scale-95'
                                    : plan.color === 'primary'
                                        ? 'bg-primary-500 text-white hover:bg-primary-400 shadow-[0_4px_20px_rgba(14,165,233,0.3)] active:scale-95'
